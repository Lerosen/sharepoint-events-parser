		...
		if (!e.fRecurrence) {
			...
			if (e.RecurrenceData.indexOf("<monthlyByDay ") != -1) {
				...
				while (loop) {
					...
					if (nd.getTime() >= start.getTime() && nd.getTime() <= end.getTime() && nd.getMonth() == init.getMonth()) { //make sure the new date calculated actually falls within the current month (sometimes there may be no 4th instance of a day)
						var ed = new Date(nd);
						ed.setSeconds(ed.getSeconds() + e.Duration);
						var ni = this.cloneObj(e);
						ni.EventDate = new Date(nd);
